...
const mapBooking = (row: any): Booking => ({
  ...
  maidId: row.maid_id,
  workStartDate: row.work_start_date instanceof Date ? new Date(row.work_start_date.getTime() + 12 * 60 * 60 * 1000).toISOString().substring(0, 10) : row.work_start_date ? String(row.work_start_date).substring(0, 10) : '',
  workEndDate: row.work_end_date instanceof Date ? new Date(row.work_end_date.getTime() + 12 * 60 * 60 * 1000).toISOString().substring(0, 10) : row.work_end_date ? String(row.work_end_date).substring(0, 10) : '',
...
export const db = {
  ...
  getContractsForUser: async (userId: string, role: string): Promise<ContractGroup[]> => {
    ...
    return rows.map((r: any): ContractGroup => ({
      ...
      workStartDate: r.work_start_date instanceof Date ? new Date(r.work_start_date.getTime() + 12 * 60 * 60 * 1000).toISOString().substring(0, 10) : r.work_start_date ? String(r.work_start_date).substring(0, 10) : '',
      workEndDate: r.work_end_date instanceof Date ? new Date(r.work_end_date.getTime() + 12 * 60 * 60 * 1000).toISOString().substring(0, 10) : r.work_end_date ? String(r.work_end_date).substring(0, 10) : '',
