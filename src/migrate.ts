...
async function migrate() {
  ...
  // Step 25: Migrate users.leaves TEXT[] column to a dedicated maid_leaves table
  console.log('\n--- Step 25: maid_leaves table ---');

  // 25a: Create the table
  await sql(`
  ...
  // 25b: Migrate existing leaves from users.leaves array (handles both "date" and "date:TYPE" formats)
